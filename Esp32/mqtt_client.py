    ...
    def subscribe(self, topic, qos=0):
        ...
        while 1:
            ...
            if op == 0x90:
                resp = self.sock.read(4)
                print(op)
                print(resp[1] == pkt[2] and resp[2] == pkt[3])
...
"""
Class to run all MQTT features:
    - connect to a server
    - subscribe
    - publish
    - reset
...
class MQTTRunner():
    ...
    def connect(self):
        ...
        try:
            self.client = MQTTClient(CLIENT_ID, config.MQTT_BROKER,user=config.MQTT_USERNAME, password=config.MQTT_API, port=1883) 
            self.client.DEBUG = True
            self.client.set_callback(self.sub_cb) 

            if not self.client.connect(clean_session=False):
                print("New session being set up")
                self.client.subscribe(topic=MQTT_TOPIC_STATE) 
        except:
